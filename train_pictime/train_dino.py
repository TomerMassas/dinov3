...
def add_version_suffix(args):
    """
    Appends V(i) to args.output_dir, where i is the next available version number.
    Checks existing V1, V2, ... folders and picks the next one.
    """
...
def main():
    ...
    # ncols: fixed width for better nohup compatibility
    pbar = tqdm(
        total=max_iters,
        desc="Training",
        unit="iter",
        file=sys.stdout,
        mininterval=300.0,  # Update every 30 seconds at most
        dynamic_ncols=False,
        ncols=100,
    )
